// CryptoSentinel - Production Engine Entrypoint
...
const CryptoArbitrageEngine = require('./engine');
const engine = new CryptoArbitrageEngine();
function handleApi(req, res, pathname, body) {
  if (req.method === 'POST' && pathname === '/api/arbitrage/calculate') {
    const data = JSON.parse(body || '{}');
    const result = engine.evaluateArbitrage(data.exA, data.priceA, data.exB, data.priceB, data.threshold);
...
if (require.main === module) {
  startServer(PORT, () => {
    console.log('CryptoSentinel server running on port ' + PORT);
  });
}
